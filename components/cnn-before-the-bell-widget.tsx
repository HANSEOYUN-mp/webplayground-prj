    ...
    <div className="w-full flex flex-col h-auto md:h-[360px] bg-card border border-border rounded-none overflow-hidden transition-colors duration-300 hover:bg-neutral-50/50 relative">
      ...
      <div className="flex items-center justify-between bg-secondary/50 px-4 py-2 border-b border-border shrink-0">
        <span className="text-[11px] font-bold text-black dark:text-white tracking-wider flex items-center gap-1.5 font-sans">
          <Newspaper className="w-3.5 h-3.5 text-black dark:text-white" /> CNN Before the Bell
        </span>
      ...
      {loading ? (
        ...
        <div className="flex-1 overflow-y-auto p-3 custom-scrollbar custom-scrollbar-beforethebell">
          <div className="flex flex-col">
            {items.map((news, i) => (
              <div
                key={i}
                className="flex items-center gap-2 py-2 border-b border-border/10 last:border-0 hover:bg-secondary/20 px-2 -mx-2 transition-colors group"
              >
                <span className="text-[9px] text-primary font-extrabold font-mono shrink-0 select-none bg-primary/10 border border-primary/20 px-1.5 py-0.5">
                ...
                <a
                  ...
                  className="text-[11.5px] font-bold text-foreground leading-snug hover:text-primary hover:underline transition-colors font-sans flex-1 min-w-0 line-clamp-2"
                >
                  {news.title}
                </a>
              </div>
